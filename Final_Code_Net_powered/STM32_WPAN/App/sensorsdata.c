...
typedef struct{
  uint16_t  SensorsdataSvcHdle;                  /**< Sensorsdata Service Handle */
}SENSORSDATA_Context_t;
...
static SENSORSDATA_Context_t SENSORSDATA_Context;
...
static SVCCTL_EvtAckStatus_t SENSORSDATA_EventHandler(void *p_pckt);
 ...
 */
static SVCCTL_EvtAckStatus_t SENSORSDATA_EventHandler(void *p_Event)
{
...
void SENSORSDATA_Init(void)
{
   ...
   */
  SVCCTL_RegisterSvcHandler(SENSORSDATA_EventHandler);

  /**
   * SensorsData
   *
   * Max_Attribute_Records = 1 + 2*0 + 1*no_of_char_with_notify_or_indicate_property + 1*no_of_char_with_broadcast_property
   * service_max_attribute_record = 1 for SensorsData +
   *                              = 1
  ...
  uuid.Char_UUID_16 = 0xeeee;
  ret = aci_gatt_add_service(UUID_TYPE_16,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(SENSORSDATA_Context.SensorsdataSvcHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    LOG_INFO_APP("  Fail   : aci_gatt_add_service command: SensorsData, error code: 0x%x \n\r", ret);
  }
  else
  {
    LOG_INFO_APP("  Success: aci_gatt_add_service command: SensorsData \n\r");
  }
